...
class PathRegistry:
    ...
    def setup(cls, path_call_file: str, service: bool = True):
        ...
        if not service:
            cls.PATH_ROOT = Path(__file__).absolute().parent.parent
...
def setup_logging():
    ...
    for handler_dic in cfg["handlers"].values():
        if "filename" in handler_dic:
            handler_dic["filename"] = str(
                PathRegistry.PATH_LOGS / Path(handler_dic["filename"]).name
            )

    logging.config.dictConfig(cfg)


def setup(call_file, service: bool = True):
    PathRegistry.setup(call_file, service=service)
    setup_logging()
